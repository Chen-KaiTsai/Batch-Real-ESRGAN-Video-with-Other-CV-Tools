...
for mp4_file in mp4_files:
    ...
    while True:
        ...
        udenoised_frame = cv2.fastNlMeansDenoisingColored(uframe, None, 10, 10, 7, 15)
        denoised_frame = cv2.UMat.get(udenoised_frame)
        vout.write(denoised_frame)
        finished_count += 1
    ...
    result_path = video_path.replace(".mp4", "_denoised.mp4")
    fvideo = ffmpeg.input(video_path)
